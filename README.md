...
```text
cmd/<context>/main.go                        # composition root (scaffold-once)
internal/<context>/
...
project to turn the injector stubs into concrete `wire_gen.go`. Because DI is
independent of `cmd`, you can enable it even with `cmd.mode: off` and consume
the provider sets from your own composition root.

**`generate.cmd.mode`** — which binaries are emitted:

| `cmd.mode` | Output |
|---|---|
| `per-context` (default) | One `cmd/<context>/main.go` per bounded context (a microservice per context). |
| `mono` | A single `cmd/<module>/main.go` wiring every context. |
| `custom` | One binary per `cmd.groups` entry — capture chosen contexts into separate binaries. |
| `off` | No `cmd/` files — you wire the application yourself. |

```yaml
generate:
  di: { enabled: true, provider: wire }
  cmd:
    mode: custom
    groups:
      - name: commerce                 # → cmd/commerce
        contexts: [OrderContext, PaymentContext]
      - name: catalog                  # → cmd/catalog
